...
/* =========================================
   OFFICIO – OUTPUT VISIBILITY HANDLER
========================================= */

function officioShowOutput(outputId) {
    const output = document.getElementById(outputId);
    if (!output) return;

    const block = output.closest(".output-block");
    if (!block) return;
...
/* =========================================
   AUTO-SHOW OUTPUT AFTER GENERATION
========================================= */

const observer = new MutationObserver(mutations => {
    mutations.forEach(m => {
        if (m.target.textContent.trim()) {
            officioShowOutput(m.target.id);
        }
    });
});

["output", "emailOutput", "textSummaryOutput"].forEach(id => {
...
/* 🔁 Trigger sobald Output Inhalt bekommt */
["output", "emailOutput", "textSummaryOutput"].forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;
...
/* =========================================
   OFFICIO – BUTTON LOADING HANDLER
========================================= */
...
document.addEventListener("click", e => {
  ...
  // kleines visuelles "Ja, hat geklappt"
  setTimeout(() => {
    btn.classList.remove("copied");
  }, 400);
});
...
/* =========================================
   OFFICIO – EMAIL TOOL HARD RESET (FINAL)
   Killt ALLE alten Listener
========================================= */
(() => {
  const oldBtn = document.getElementById("emailGenerateBtn");
  if (!oldBtn) return;

  // 🔥 Button komplett ersetzen
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (hasFile) {
        ...
        console.log("📤 EMAIL → FILE ENDPOINT");
      ...
      // ================= TEXT =================
        ...
        console.log("📤 EMAIL → TEXT ENDPOINT");
...
/* =========================================
   OFFICIO – EMAIL TOOL FINAL ROUTER FIX
   ID-unabhängig, 100 % zuverlässig
========================================= */
(() => {
  const tool = document.getElementById("tool-email");
  if (!tool) return;

  const oldBtn = tool.querySelector("button");
  if (!oldBtn) return;

  // 🔥 Button vollständig resetten
  const btn = oldBtn.cloneNode(true);
  oldBtn.parentNode.replaceChild(btn, oldBtn);
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (hasFile) {
        console.log("📤 EMAIL → /email-reply-file");
